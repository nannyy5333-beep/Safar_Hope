...
class CRMManager:
    ...
    def track_customer_journey(self, user_id):
        ...
        # Регистрация
        user = self.db.execute_query('SELECT created_at FROM users WHERE id = %s', (user_id,))[0]
        events.append({
            'type': 'registration',
            'date': user[0],
        ...
        first_view = self.db.execute_query('''
            SELECT MIN(created_at) FROM cart WHERE user_id = %s
        ''', (user_id,))[0]
    ...
    def get_churn_risk_customers(self):
        ...
        at_risk_customers = self.db.execute_query('''
            ...
            WHERE u.is_admin = false AND o.status != 'cancelled'
    ...
    def calculate_customer_satisfaction_score(self, user_id):
        ...
        avg_rating = self.db.execute_query(
            'SELECT AVG(rating) FROM reviews WHERE user_id = %s',
            (user_id,)
        )[0][0]
        ...
        repeat_purchase_rate = self.db.execute_query('''
            SELECT COUNT(*) FROM orders WHERE user_id = %s AND status != 'cancelled'
        ''', (user_id,))[0][0]
