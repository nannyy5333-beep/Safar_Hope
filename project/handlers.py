...
class MessageHandler:
    ...
    def handle_message(self, message):
        ...
        try:
            ...
            # ВАЖНО: сначала проверяем состояния пользователя (регистрация и т.д.)
            if telegram_id in self.user_states:
                self.handle_user_state(message)
                return

            # Команды, доступные без регистрации
            ALLOWED_BEFORE_REG = ['/start', '/help', '🌍 Сменить язык', 'ℹ️ Помощь', 'ℹ️ Yordam', '📞 Связаться с нами', "📞 Biz bilan bog'lanish"]
            ...
            is_registered = False
            if user_data:
                # Считаем зарегистрированным если есть телефон или флаг is_registered
                phone = user_data[0][3]
                is_reg_flag = user_data[0][8] if len(user_data[0]) > 8 else False
                is_registered = bool(phone) or bool(is_reg_flag)

            # Если пользователь не зарегистрирован и команда не разрешена без регистрации
            if not is_registered and text not in ALLOWED_BEFORE_REG:
    ...
    def handle_registration_language(self, message):
        ...
        if user_id:
            ...
            self.db.get_user_loyalty_points(user_id)
    ...
    def send_registration_prompt(self, chat_id):
        ...
        self.bot.send_message(chat_id, prompt_text, guest_keyboard)
...
"""
        ...
        if category:
            ...
            if subcategories:
                ...
                products = self.db.execute_query(
                    'SELECT * FROM products WHERE category_id = %s AND is_active = true ORDER BY name LIMIT 30',
                    (category_id,)
                )
                if products:
    ...
    def handle_subcategory_selection(self, message):
        ...
        subcategory = self.db.execute_query(
            'SELECT id FROM subcategories WHERE name = %s AND is_active = true',
            (subcategory_name,)
        )
    ...
    def handle_product_selection(self, message):
        ...
        product = self.db.execute_query(
            'SELECT * FROM products WHERE name = %s AND is_active = true',
            (product_name,)
        )
    ...
    def handle_callback_query(self, callback_query):
        ...
        try:
            ...
            if data == 'back_to_categories':
                ...
                if cid:
                    # Показ подкатегорий
                    cat_row = self.db.execute_query('SELECT name FROM categories WHERE id=%s', (cid,))
                ...
                if sid:
                    ...
                    sub_row = self.db.execute_query('SELECT name FROM subcategories WHERE id=%s', (sid,))
    ...
    def handle_rate_product(self, callback_query):
        ...
        try:
            ...
            purchased = self.db.execute_query('''
                ...
                WHERE o.user_id = %s AND oi.product_id = %s AND o.status != 'cancelled'
...
def handle_seller_products(self, message):
    ...
    data['products'] = text
    try:
        admins = self.db.execute_query('SELECT telegram_id FROM users WHERE is_admin = true')
        if not admins:
