...
class InventoryManager:
    ...
    def reserve_stock(self, product_id, quantity, order_id):
        ...
        self.db.execute_query(
            'UPDATE products SET stock = stock - ? WHERE id = %s',
            (quantity, product_id)
        )
    ...
    def release_reservation(self, order_id):
        ...
        reservations = self.db.execute_query(
            'SELECT product_id, quantity FROM stock_reservations WHERE order_id = %s',
            (order_id,)
        )
        ...
        # Удаляем резервы
        self.db.execute_query(
            'DELETE FROM stock_reservations WHERE order_id = %s',
            (order_id,)
        )
    ...
    def trigger_automatic_reorder(self, product_id):
        ...
        recent_order = self.db.execute_query('''
            ...
            WHERE product_id = %s AND created_at >= datetime('now', '-7 days')
        ''', (product_id,))
    ...
    def create_purchase_order(self, product_id, quantity, supplier_id):
        ...
        supplier = self.db.execute_query(
            'SELECT name, contact_email, cost_per_unit FROM suppliers WHERE id = %s',
            (supplier_id,)
        )
    ...
    def process_incoming_shipment(self, purchase_order_id, received_quantity, condition='good'):
        ...
        purchase_order = self.db.execute_query(
            'SELECT product_id, quantity, cost_per_unit FROM purchase_orders WHERE id = %s',
            (purchase_order_id,)
        )
        ...
        self.db.execute_query(
            'UPDATE purchase_orders SET status = %s, received_quantity = ? WHERE id = %s',
