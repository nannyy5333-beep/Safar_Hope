...
# === Telegram credentials (from environment only) ===
BOT_TOKEN = os.getenv('TELEGRAM_BOT_TOKEN')
POST_CHANNEL_ID = os.getenv('POST_CHANNEL_ID')

# Only validate when actually running the bot (not during import checks)
if __name__ != "__main__" and os.getenv('CHECK_ENV_VARS', 'true').lower() == 'true':
    if not BOT_TOKEN:
        raise ValueError("TELEGRAM_BOT_TOKEN environment variable is required")
    if not POST_CHANNEL_ID:
        raise ValueError("POST_CHANNEL_ID environment variable is required")
...
# Настройки базы данных (Render PostgreSQL)
DATABASE_CONFIG = {
