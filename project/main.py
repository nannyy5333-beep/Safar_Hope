...
import sys
import threading
from postgres_db import PostgresManager as DatabaseManager, get_db
...
from tests.health_check import HealthMonitor
        ...
        if admin_telegram_id:
            try:
                ...
                existing_admin = self.db.execute_query(
                    'SELECT id, is_admin FROM users WHERE telegram_id = %s',
                    (admin_telegram_id,)
                )
